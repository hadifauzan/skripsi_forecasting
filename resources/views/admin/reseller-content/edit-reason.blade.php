...
@section('title', 'Edit Alasan Reseller')
    ...
    <div class="px-6 py-8">
        ...
        <div class="flex items-center justify-between py-5 font-nunito">
            <h2 class="font-nunito font-bold text-2xl text-[#614DAC]">
                Edit
                {{ $section === 'reseller-why-join-title' ? 'Why Join Us Title' : ucfirst(str_replace('-', ' ', $section)) }}
            </h2>
        ...
        @if ($section === 'reseller-why-join-title')
            {{-- Why Join Title Form --}}
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">

                <!-- Form Content -->
                <form
                    action="{{ route('admin.reseller-content.section.update', ['sectionKey' => 'reasons', 'section' => $section]) }}"
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="title" name="title"
                                ...
                                placeholder="Contoh: Why Join Us (opsional)">
                            @error('title')
        ...
        @elseif (str_contains($section, 'reseller-benefit-'))
            {{-- Individual Benefit Edit Form --}}
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                <!-- Header Form -->
                <div class="bg-[#785576] p-6">
                    <div class="flex items-center text-white">
                        ...
                        <div>
                            <h3 class="text-xl font-semibold">Edit {{ ucfirst(str_replace('-', ' ', $section)) }}</h3>
                            <p class="text-purple-100 text-sm">Manfaat bergabung sebagai reseller</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="title" name="title"
                                value="{{ old('title', $content->title ?? '') }}"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#785576] focus:border-transparent transition-all duration-200 hover:border-[#785576]/50"
                                placeholder="Contoh: Produk Best Quality (opsional)">
                            @error('title')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="body" name="body" rows="4"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#785576] focus:border-transparent transition-all duration-200 resize-none hover:border-[#785576]/50"
                                placeholder="Masukkan deskripsi benefit ini...">{{ old('body', $content->body ?? '') }}</textarea>
                            @error('body')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
