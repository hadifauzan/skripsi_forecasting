...
@section('content')
    ...
    <div class="px-6 py-8">
        ...
        {{-- Main Form --}}
        @if ($isCreate ?? false)
            <form method="POST" action="{{ route('admin.reseller-content.section.store', 'steps') }}">
                @csrf
                <input type="hidden" name="section_type" value="step">
                <input type="hidden" name="type_of_page" value="partner">
                <input type="hidden" name="section" value="{{ $section }}">
            @else
                <form method="POST"
                    action="{{ route('admin.reseller-content.section.update', ['sectionKey' => 'steps', 'section' => $section]) }}">
                    @csrf
                    @method('PUT')
        @endif
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div class="p-6">
                @if ($section === 'reseller-how-to-join-title')
                    {{-- Title Section --}}
                    <div class="mb-6">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="body" name="body" rows="3"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#785576] focus:border-transparent"
                            placeholder="Masukkan deskripsi section">{{ old('body', $content->body ?? '') }}</textarea>
                    </div>
                @else
