...
@section('title', 'Edit Header Keluarga Gentle Living')
    ...
    <div class="px-6 py-8">
        ...
        <div class="flex items-center justify-between py-5 font-nunito">
            <h2 class="font-nunito font-bold text-2xl text-[#614DAC]">
                Edit Header Keluarga Gentle Living
            </h2>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div class="bg-[#785576] p-6 text-white">
                <h3 class="text-xl font-semibold">Form Edit Header</h3>
                <p class="text-purple-100 text-sm">Perbarui judul dan deskripsi section Keluarga Gentle Living</p>
            </div>

            <form action="{{ route('admin.about-us-content.update-family-header') }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" id="title" name="title"
                            value="{{ $familyHeader->title ?? 'Keluarga Gentle Living' }}"
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="4"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#785576]">{{ $familyHeader->body ?? 'Bergabunglah dengan ribuan keluarga Indonesia yang telah mempercayakan nutrisi terbaik untuk si kecil kepada Gentle Living.' }}</textarea>
                        @error('body')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
