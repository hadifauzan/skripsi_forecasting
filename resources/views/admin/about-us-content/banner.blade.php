...
@section('title', 'About Us - Banner Management')
            ...
            <div class="bg-[#6B7280] p-6">
                <div class="flex items-center text-white">
                    <div>
                        ...
                        <p class="text-purple-100 text-sm">Modifikasi banner About Us sesuai kebutuhan terkini</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 font-nunito">
                <div class="space-y-4">
                        ...
                        <div
                            ...
                            <div
                                ...
                                <div class="flex items-center space-x-2 ml-4">
                                    <!-- Edit Button -->
                                    <button type="button"
                                        onclick="window.location.href='{{ route('admin.about-us-content.edit-banner', 'hero') }}'"
                            ...
                            <div class="px-6 pb-6">
                                <div class="bg-white rounded-lg p-4 border-l-4 border-purple-500">
                                    <h5 class="font-bold text-gray-800 mb-2">{{ $heroContent->title }}</h5>
                                    <p class="text-gray-700 leading-relaxed">{{ $heroContent->body }}</p>
                                </div>
                                <div class="flex justify-between items-center mt-3 text-xs text-[#72C7B4]">
                                    <span>Diperbarui:
                                        {{ $heroContent->updated_at ? $heroContent->updated_at->format('d M Y H:i') : 'Belum diperbarui' }}</span>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    @if ($bannerImages->count() > 0)
                        @foreach ($bannerImages as $index => $item)
                            <div
                                class="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl border border-purple-200 overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                                <div
                                    class="w-full px-6 py-4 flex justify-between items-center group transition-all duration-300 hover:bg-white/50">
                                    <div class="flex items-center space-x-4 flex-1">
                                        ...
                                        <div class="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                                            @if ($item && $item->image)
                                                <img src="{{ Storage::url($item->image) }}" alt="Banner {{ $index + 1 }}"
                                        ...
                                        <div>
                                            <h4
                                                ...
                                                Gambar Banner {{ $index + 1 }}
                                            </h4>
                                            <p class="text-sm text-gray-500">
                                                @if ($item && $item->image)
                                                    <span class="text-green-600">✓ Gambar sudah diupload</span>
                                                @else
                                                    <span class="text-orange-500">⚠ Belum ada gambar</span>
                                                @endif
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {{-- Edit Button --}}
                                    <div class="flex items-center space-x-2 ml-4">
                                        <a href="{{ route('admin.about-us-content.edit-banner', $item ? $item->section : "banner-{$loop->iteration}") }}"
                                            class="text-blue-600 hover:text-blue-800 transition-all duration-300 p-2 rounded-full hover:bg-blue-100 hover:rotate-6"
                                            title="Edit Banner {{ $index + 1 }}">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z">
                                                </path>
