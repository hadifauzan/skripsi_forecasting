...
@section('content')
    <div class="px-6 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div class="bg-[#785576] p-6 text-white">
                <h3 class="text-xl font-semibold">Form Edit Misi</h3>
                <p class="text-purple-100 text-sm">Kelola misi perusahaan</p>
            </div>

            <form action="{{ route('admin.about-us-content.update-mission') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" id="mission_title" name="mission_title"
                        value="{{ old('mission_title', $missionContent->title ?? 'Misi') }}"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-3">
                        Poin-poin Misi <span class="text-red-500">*</span>
                    </label>

                    <div id="mission-container" class="space-y-3">
                        @if (!empty($missionData) && is_array($missionData))
                            @foreach ($missionData as $index => $mission)
                                <div class="mission-item bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <textarea name="mission[]" rows="2"
                                            ...
                                            placeholder="Masukkan poin misi...">{{ old('mission.' . $index, $mission) }}</textarea>
                                        <button type="button" onclick="removeMissionItem(this)"
                        ...
                        @else
                            <div class="mission-item bg-gray-50 rounded-lg p-4 border border-gray-200">
                                <div class="flex items-start space-x-3">
                                    ...
                                    <textarea name="mission[]" rows="2"
                                        class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none transition-all duration-300"
                                        placeholder="Masukkan poin misi...">{{ old('mission.0') }}</textarea>
                                    <button type="button" onclick="removeMissionItem(this)"
                                        class="flex-shrink-0 text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-50 transition-all duration-300"
                                        title="Hapus poin">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button type="button" onclick="addMissionItem()"
                        ...
                        <span class="font-medium">Tambah Poin Misi</span>
                    </button>
                </div>
    ...
    <script>
        let missionCounter = {{ count($missionData ?? []) > 0 ? count($missionData) : 1 }};

        function addMissionItem() {
            missionCounter++;
            const container = document.getElementById('mission-container');
            const newItem = document.createElement('div');
            newItem.className = 'mission-item bg-gray-50 rounded-lg p-4 border border-gray-200';
            newItem.innerHTML = `
                <div class="flex items-start space-x-3">
                    <div class="flex-shrink-0 w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center mt-1">
                        <span class="text-purple-600 font-semibold text-sm">${missionCounter}</span>
                    </div>
                    <textarea name="mission[]" rows="2"
                        class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none transition-all duration-300"
                        placeholder="Masukkan poin misi..."></textarea>
                    <button type="button" onclick="removeMissionItem(this)"
                        class="flex-shrink-0 text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-50 transition-all duration-300"
                        title="Hapus poin">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        function updateNumbering() {
            ...
            missionCounter = missionItems.length;
        }
    </script>
@endsection
