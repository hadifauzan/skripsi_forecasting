...
@section('content')
    <div class="p-6">
        ...
        <form action="{{ route('admin.articles.update', $article->content_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
                <div class="bg-[#785576] p-6">
                    <div class="flex items-center text-white">
                        <div>
                            <h3 class="text-xl font-semibold">Edit Artikel</h3>
                            <p class="text-purple-100 text-sm">Data akan diperbarui di tabel master_content (ID: {{ $article->content_id }})</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <select id="section" name="section" required
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->slug }}" {{ old('section', $article->section) == $category->slug ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="status" value="1" {{ old('status', $article->status) ? 'checked' : '' }}
                                        class="w-4 h-4 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-500 focus:ring-2">
                                    <span class="ml-2 text-sm text-gray-700">Aktif</span>
                                </label>
                            </div>
                            <p class="text-xs text-gray-500 mt-1">Artikel yang aktif akan ditampilkan di website</p>
                        </div>
                    </div>

                    {{-- Current Thumbnail and Upload --}}
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            {{-- Current Image --}}
                            @if($article->image)
                                <div id="current-image-container" class="bg-gradient-to-br from-blue-50 to-indigo-50 p-5 rounded-xl border-2 border-blue-300 shadow-lg">
                                    <div class="flex items-center justify-between mb-3">
                                        <div class="flex items-center space-x-2">
                                            <span class="text-2xl">📷</span>
                                            <div>
                                                <p class="text-sm font-bold text-blue-800">Thumbnail Saat Ini</p>
                                                <p class="text-xs text-blue-600">Gambar aktif</p>
                                            </div>
                                        </div>
                                        <span class="text-xs text-white bg-blue-600 px-3 py-1.5 rounded-full font-semibold shadow-md">● Aktif</span>
                                    </div>
                                    <div class="bg-white p-2 rounded-lg shadow-md">
                                        <img src="{{ asset('storage/' . $article->image) }}" alt="{{ $article->title }}" 
                                            class="w-full h-64 object-contain rounded-lg">
                                    </div>
                                    <div class="mt-3 bg-white/60 backdrop-blur-sm p-2 rounded-lg border border-blue-200">
                                        <p class="text-xs text-blue-700 text-center">
                                            <svg class="w-4 h-4 inline mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                                            </svg>
                                            Gambar ini akan dipertahankan jika tidak upload gambar baru
                                        </p>
                                    </div>
                                </div>
                            @else
                                <div class="bg-gray-50 p-5 rounded-xl border-2 border-dashed border-gray-300">
                                    <div class="flex items-center space-x-2 mb-3">
                                        <span class="text-2xl">📷</span>
                                        <p class="text-sm font-bold text-gray-700">Thumbnail Saat Ini</p>
                                    </div>
                            ...
                            {{-- New Image Preview --}}
                            <div id="new-image-preview-container" class="bg-gradient-to-br from-gray-50 to-slate-50 p-5 rounded-xl border-2 border-dashed border-gray-300">
                                ...
                                <div id="preview-empty-state" class="text-center">
                                    <div class="flex items-center justify-center mb-3">
                                        <div class="flex items-center space-x-2">
                                            <span class="text-2xl">🆕</span>
                                            <div>
                                                <p class="text-sm font-bold text-gray-700">Preview Thumbnail Baru</p>
                                    ...
                                    <div class="bg-white p-2 rounded-lg shadow-sm border-2 border-dashed border-gray-200">
                                        <div class="w-full h-64 bg-gray-100 rounded-lg flex items-center justify-center">
                                            <div class="text-center">
                                                ...
                                                <p class="text-gray-400 text-xs">Upload untuk melihat preview</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div id="preview-with-image" class="hidden">
                                    <div class="flex items-center justify-between mb-3">
                                        <div class="flex items-center space-x-2">
                                            <span class="text-2xl">🆕</span>
                                            <div>
                                                <p class="text-sm font-bold text-green-800">Thumbnail Baru</p>
                                                <p class="text-xs text-green-600">Preview sebelum upload</p>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-3 bg-white/60 backdrop-blur-sm p-2 rounded-lg border border-green-200">
                                ...
                                <div id="upload-placeholder">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-purple-600 hover:text-purple-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-purple-500">
                                            <span>{{ $article->image ? 'Upload thumbnail baru' : 'Upload thumbnail' }}</span>
                                    ...
                                    @if($article->image)
                                        <p class="text-xs text-blue-600 mt-2">💡 Upload gambar baru untuk mengganti thumbnail saat ini</p>
                                    @endif
                                </div>
                    ...
                    <div>
                        <label for="body" class="block text-sm font-medium text-gray-700 mb-2">
                            ...
                            <span class="text-xs text-gray-500 ml-2">(disimpan ke field: body)</span>
                        </label>
                        
                        <!-- Textarea untuk Isi Artikel -->
                        <textarea id="body" name="body" rows="15" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                            placeholder="Tulis isi artikel di sini...">{{ old('body', $article->body) }}</textarea>
    ...
    window.previewNewImage = function(input) {
        console.log('========================================');
        console.log('🔍 [EDIT] previewNewImage called');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('✅ DOM Ready - EDIT PAGE');
