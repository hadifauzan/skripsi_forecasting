...
@section('title', 'Kelola Benefits affiliate')
...
@section('content')
        ...
        <div class="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div class="bg-[#785576] p-6">
                <div class="flex items-center text-white">
                    <div>
                        ...
                        <p class="text-purple-100 text-sm">Kelola keuntungan yang didapat affiliate</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 font-nunito">
                <div class="space-y-4">
                        ...
                        <div
                            ...
                            <div
                                ...
                                <div class="flex items-center space-x-2 ml-4">
                                    <!-- Edit Button -->
                                    <button type="button" onclick="editBenefits('what-you-get-title')"
                            ...
                            <div class="px-6 pb-6">
                                <div class="bg-white rounded-lg p-4 border-l-4 border-purple-500">
                                    <h5 class="font-bold text-gray-800 mb-2">
                                        {{ $whatYouGetTitle->title ?? 'Judul belum diatur' }}</h5>
                                    <p class="text-gray-700 leading-relaxed">
                                        {{ $whatYouGetTitle->body ?? 'Deskripsi belum diatur' }}</p>
                                </div>
                                <div class="flex justify-between items-center mt-3 text-xs text-[#72C7B4]">
                                    <span>Diperbarui:
                                        {{ $whatYouGetTitle->updated_at ? $whatYouGetTitle->updated_at->format('d M Y H:i') : 'Belum tersedia' }}</span>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        @php
                            $item = $whatYouGet->firstWhere('section', 'what-get-' . $i);
                        @endphp
                        <div
                            class="bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl border border-purple-200 overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                            <div
                                ...
                                <div class="flex items-center space-x-2 ml-4">
                                    <!-- Edit Button -->
                                    <button type="button" onclick="editBenefits('what-get-{{ $i }}')"
                                        class="text-blue-600 hover:text-blue-800 transition-all duration-300 p-2 rounded-full hover:bg-blue-100 hover:rotate-6"
    ...
    <script>
        // Edit Benefits - redirect to edit page with data
        function editBenefits(section) {
            // Redirect to edit page
            window.location.href = `{{ route('admin.affiliate-content.benefits.edit', ':section') }}`.replace(':section',
                section);
        }
