    ...
    <div class="px-6 py-8">
        ...
        <div class="flex items-center justify-between py-5 font-nunito">
            <h2 class="font-nunito font-bold text-2xl text-[#614DAC]">
                Edit {{ $section === 'perfect-for-title' ? 'Perfect For Title' : ucfirst(str_replace('-', ' ', $section)) }}
            </h2>
        ...
        {{-- Main Form --}}
        <form method="POST" action="{{ route('admin.affiliate-content.update', $section) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    @else
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" id="title" name="title"
                                value="{{ old('title', $content->title ?? '') }}"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#785576] focus:border-transparent"
                                placeholder="Contoh: Influencer Media Sosial (opsional)">
                        </div>

                        <div class="mb-6">
                            <label for="body" class="block text-sm font-medium text-gray-700 mb-2">
                                Deskripsi Target affiliate
                            </label>
                            <textarea id="body" name="body" rows="4"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#785576] focus:border-transparent"
                                placeholder="Jelaskan mengapa cocok untuk jenis affiliate ini (opsional)">{{ old('body', $content->body ?? '') }}</textarea>
                        </div>
                    @endif
                </div>
